...
#API key connection
def Api_connect():
    Api_Id="AIzaSyDhCmpvx6NYi84Z7MWal-pEy2KGfhlug_Q"

    api_service_name="youtube"
    api_versions="v3"

    youtube=build(api_service_name,api_versions,developerKey=Api_Id)
...
def comments_tables():
    ...
    drop_query='''drop table if exists lcomments'''
    cursor.execute(drop_query)
    mydb.commit()
...
if question=="1. All the videos and the channel name":
    ...
    mydb.commit()
    t7=cursor.fetchall()
    df7=pd.DataFrame(t7,columns=["channel_name","total views"])
    st.write(df7)
